class Admin::OrdersController < AdminController
  ...
  def index
    ...
    @not_fullfiled_orders = Order.where(fullfiled: false).order(created_at: :asc)
    @fullfiled_orders = Order.where(fullfiled: true).order(created_at: :asc)
  end
  ...
  private
    ...
    def admin_order_params
      params.expect(order: [ :customer_email, :fullfiled, :total, :addres ])
    end
end
